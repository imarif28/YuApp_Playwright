...
import { adminData } from '../../../../data/adminData';
    ...
    const id_marketing = '90';

    test('Mereset akun marketing yang akan ditugaskan', async ({ page }) => {
        const loginPage = new LoginPage(page);
        const adminPage = new AdminPage(page);

        await loginPage.goto();
        await loginPage.login(process.env.ADMIN_USERNAME!, process.env.ADMIN_PASSWORD!);
        await adminPage.editMarketingInCustomer(customer_name, id_marketing);
        await adminPage.verifysuccessNotification();
        await adminPage.verifyEditMarketingInCustomer(customer_name, id_marketing);
    });
